...
FIELDS = ["artist", "album", "ru_price_rub", "ru_n", "ru_p25", "ru_p75",
...
def build(target=3.0, use_net=True):
    rate, stale = usdrub()
    ...
    for (artist, album), m in sorted(meds.items()):
        ...
        if token is not None:
            ...
            time.sleep(2.5)          # без токена лимит 25 в минуту
        discs = m.get("discs")
...
def main():
    ...
    a = ap.parse_args()
    rows, rate, stale = build(a.target, not a.no_net)
    ...
    if bad:
        print(f"\n{len(bad)} из {len(rows)}: {a.target}x к полной "
              f"себестоимости недостижима при любой цене на eBay — "
              f"карго больше трети московской цены.")
    return 0
