...
# Доставка до Москвы по уточнённым числам владельца, 13.09.2026:
# $11 за одиночник, $22 за двойник. Это не вилка, а его тариф, поэтому
# обе границы совпадают, и множитель снова одно число.
#
# Против измеренного нами карго ($9.46 за одиночник в партии десять)
# это чуть ХУЖЕ, а не лучше: граница цены товара под 3x к верхним 5%
# рынка сжимается с $13.48 до $11.94. Против прежней вилки $12-18 и
# $35-50 за бокс — заметно лучше, особенно на многодисковых.
#
# Для трёх пластинок и больше тариф владельцем не назван, поэтому он
# продолжается линейно от двойника: $11 за диск. Это допущение, и оно
# помечено здесь, чтобы его не приняли за измерение.
FWD_PER_DISC = 11.0


def fwd_usd(discs):
    return FWD_PER_DISC * max(1, int(discs or 1))
...
def main():
    ...
    floor_rub = fwd_usd(1) * a.target * rate
