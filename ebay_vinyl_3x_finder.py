...
Что реально доступно и используется здесь:
  ...
  - НАЙДЕНО 30.08.2026 (двойной ручной разбор пользователя, Bennie
    Green — Walking Down, PRLP 7049): margin_condition_adjusted ВЕЗДЕ
    в этом скрипте считается от МИРОВОГО рынка (текущие листинги/
    продажи Discogs marketplace, см. выше про degraded mode) — а
    ПРОДАВАТЬ на Discogs ИЗ РОССИИ нельзя (buy-only с 2022г.). Это не
    баг конкретного лота, а системное ограничение методологии: любая
    margin здесь отвечает на вопрос "насколько этот пресс ценен
    глобально", а НЕ "сколько реально выручу в Москве". Реальный канал
    сбыта (МаркетВинила/Мешок/VK-группы/скупка) на порядки тоньше и
    дешевле мирового рынка, особенно для одиночных player-копий с
    неидеальным состоянием — конкретный пример: скрипт посчитал 17.97x
    от мировой degraded-цены $453, а реалистичная выручка в Москве для
    этого конкретного экземпляра (винил G) оказалась 8000-14000₽
    (~$90-165), что при пересчёте по правилу 3x даёт потолок ставки
    ~$21-25, а не тот, что подразумевает мировая margin. Данных о
    реальных ценах российского вторичного рынка винила ни в одном API,
    которым пользуется этот скрипт, нет и взяться неоткуда — это не
    "не реализовано", а честно недоступный источник данных (см. ту же
    оговорку в известных ограничениях конфига про Discogs sold-price).
    Практический вывод: перед реальной ставкой ВСЕГДА пересчитывать
    правило 3x от реалистичной цены продажи В МОСКВЕ (не от margin из
    CSV) — особенно на лотах с высокой margin, где степень оптимизма
    мирового рынка накапливается больше всего.
======================================================================
...
CATALOG_PATTERNS = [
    ...
    # НАЙДЕНО 29.08.2026 (ручной разбор пользователя, Pharoah Sanders —
    # Love In Us All, ABC/Impulse ASD-9280): поздний ABC/Impulse-период
    # (после смены владельца с ABC) использовал префикс "ASD" (stereo),
    # не только "AS" — паттерн ниже его не ловил вообще, catno оставался
    # None, и фаззи-поиск по названию давал НЕСТАБИЛЬНЫЙ топ-результат
    # между вызовами (итальянский пресс в одном вызове, правильный US в
    # другом, при абсолютно одинаковом запросе) — резолв не то что
    # неточный, а буквально непредсказуемый от прогона к прогону.
    r"\bASD?[\s-]?\d{2,5}\b",                    # Impulse! AS-9120 / AS-66 / ASD-9280
    ...
    # НАЙДЕНО 29.08.2026 (ручной разбор пользователя, Mariachi
    # Brass/Chet Baker — A Taste Of Tequila, WPS-21839): World Pacific
    # не имел паттерна вообще — без catno-извлечения discogs_resolve_
    # release() ушёл в чистый фаззи-поиск по названию, где Discogs
    # ранжировал АВСТРАЛИЙСКИЙ пресс с ДРУГИМ catno выше настоящего
    # точного совпадения (US, WPS-21839, обнаружен только на 3-й позиции
    # результатов) — top=results[0] взял неверный, все прессы дальше
    # дизамбигуировались вокруг чужого catno. WP/WPS — достаточно
    # специфичный двух/трёхбуквенный префикс (риск ложных совпадений
    # того же порядка, что уже принятый "AS" для Impulse! выше).
    r"\bWPS?[\s-]?\d{3,5}\b",                    # World Pacific WP-1839 / WPS-21839
]
...
# НАЙДЕНО 29.08.2026 (ручной разбор пользователя, Miles Davis Quintet —
# Steamin', Prestige PRLP 7200): Discogs хранит catno оригинального
# 1961г. пресса ГОЛЫМ числом ('7200', без буквенного префикса), в то
# время как из текста лота (и с самого лейбла пластинки) извлекается
# 'PRLP 7200' — normalize_catno() их никогда не признает равными.
# same_catno систематически терял оригинал, оставляя только те немногие
# карточки Discogs, что ТОЖЕ хранят полный префикс (в этом случае —
# современный стерео-рессиз, catno='PRLP 7200' — а не оригинал). Список
# префиксов ниже — тот же набор, что уже распознаёт CATALOG_PATTERNS для
# ИЗВЛЕЧЕНИЯ; это не открывает риск голых-чисел-коллизий (см. известные
# ограничения про Contemporary/Black Saint, где голый номер сознательно
# НЕ извлекается из текста лота вообще) — сравнение применяется только
# внутри уже найденной по catno= группы результатов Discogs (см.
# discogs_resolve_release), то есть Discogs сам уже ограничил её одной
# каталожной серией, снимать префикс здесь безопасно.
CATNO_PREFIX_STRIP_RE = re.compile(
    r"^(ecm|blp|bst|asd|as|v6|v|sd|cs|sn|prlp|prst|pr|rlp|cti|scs|scc|ojc|wps|wp)"
)
...
def discogs_resolve_release(item, cfg):
    ...
    if dm.get("require_exact_release_match", True) and catno:
        ...
        # НАЙДЕНО 29.08.2026 (ручной разбор пользователя, Miles Davis
        # Quintet — Steamin', PRLP 7200): строгое сравнение (нормализация
        # БЕЗ снятия префикса) пропускает оригинал 1961г. — Discogs хранит
        # его catno ГОЛЫМ числом ('7200'), без префикса 'PRLP', который мы
        # извлекли из текста лота. Строгое сравнение НЕ было пустым (оно
        # находило 2 карточки — обе современные рессизы, у которых Discogs
